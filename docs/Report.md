...
In MATLAB, we simulated the lungs in 3D. Using the data from the CSV file generated by the C code, we visualized the number of cells at each time step by coloring the lungs accordingly. We then created frames from this process and combined them into a GIF. In order to make a progression that would be visible yet stay in our domain we had to apply a scale between the size of a 'cell' and the size of our voxels; we used a scale of 14 so 1 voxel is equal to 100 cancer cells. 

This MATLAB code simulates the progression of lung cancer in both a healthy individual and a smoker by generating 3D visualizations of lung structures and tumor growth over time. The code begins by creating a 3D mesh representing the lungs, modeled as two ellipsoids for the right and left lungs. These are constructed using the meshgrid function for generating coordinates and mathematical equations for the ellipsoids, with the lung volume being defined by boolean masks (mask_right and mask_left) that identify the points within the lung geometry.
...
To visualize the tumor growth, the code creates frames at each time step. For each frame, it calculates the number of tumor voxels by dividing the cancer cell count by a scale factor (to represent cancerous cells visually in the 3D lung geometry). The save_frame function generates these visualizations by creating a 3D isosurface mesh for the lungs and tumor volume at each time step. The tumor is represented in red, while the lung tissue is drawn with a translucent light pink color. The function also generates and saves each frame as a PNG image and appends these frames to a GIF file that shows the progression of the tumor growth over time.

The script then saves these images as a series of frames in a specified folder and compiles them into a GIF that shows the evolution of cancer in the lungs over time for both the healthy and smoker cases. The use of the isosurface function is crucial here, as it allows the creation of 3D surfaces representing the boundaries of tumor regions in the lung voxel grid. The imwrite function is used to compile the images into an animated GIF, where each frame represents a snapshot of the tumor's growth.

In summary, this code models and visualizes the progression of cancer in the lungs, comparing the tumor growth over time for a healthy individual versus a smoker, creating a dynamic visual representation of how lifestyle factors influence health.

# Results

<p float="left">
  <img src="https://raw.githubusercontent.com/shirelepfl/CMT-Project/main/images/csv-healthy.png" alt="Healthy lung values" width="45%" />
  <img src="https://raw.githubusercontent.com/shirelepfl/CMT-Project/main/images/csv-smoker.png" alt="Smoker lung values" width="45%" />
</p>
